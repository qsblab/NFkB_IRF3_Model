% Estimation of Interferon mRNA Level: Promoter Binding Activity when both 
% IRF-3 and NFkB are bound upstream of Type I IFN genes
% =========================================================================
...
for idx = 1:numLines
    ...
    if ~isempty (NFkB_values{idx})
        ...
        results{idx,1} =  ((((nfkb.^n)./((k_nfkb.^n)+(nfkb.^n))).*(1 -((irf3.^n)./((k_irf.^n)+(irf3.^n)))))+(((irf3.^n)./((k_irf.^n)+(irf3.^n))).*(1 - ((nfkb.^n)./((k_nfkb.^n)+(nfkb.^n)))))+(((nfkb.^n)./((k_nfkb.^n)+(nfkb.^n))).*((irf3.^n)./((k_irf.^n)+(irf3.^n))))).*100;
...
for i = 1:length(polyIC_indices) % PolyI:C indices
    for j = 1:2 %Time indices
        ...
        if ~isempty(result_idx)
            ...
            subplot(1,2,plot_index);                                 % Command to create 18 subplots in the same figure present subplot represent current index of cell array accessed 
