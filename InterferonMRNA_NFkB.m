% Estimation of Interferon mRNA Level: Promoter Binding Activity 
% when only NFkB is bound upstream of Type I IFN genes
% =========================================================================
...
for idx = 1:numLines
    ...
    if ~isempty (NFkB_values{idx})
        ...
        results{idx,1} = (((nfkb.^n)./((k_nfkb.^n)+(nfkb.^n))).*(1-((irf3.^n)./((k_irf.^n)+(irf3.^n))))).*100;
    else
        results{idx}=[];
    end
